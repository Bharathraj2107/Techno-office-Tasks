...
class SimpleMotorEmulator:
    def __init__(
        ...
        self.limits = limits if limits else MotorLimits()
    ...
    async def move_to_position(self, position: float):
        ...
        self.movement_task = asyncio.create_task(self._smooth_movement())
    ...
    async def _smooth_movement(self):
        ...
        try:
            ...
            while (self.speed < self.target_speed and 
                ...
                self.speed = min(self.speed + self.limits.acceleration * 0.1, 
                               self.target_speed, 
...
async def motor_callback(motor_id: int, state: MotorState, position: float):
    ...
    print(f"Callback: Motor {motor_id} is now {state.name} at position {position:.2f}")
...
async def main():
    ...
    # Final position
    print(f"Final position: {await motor.get_position()}")
